local old_set_debuff = Card.set_debuff
function Card:set_debuff(should_debuff)
    old_set_debuff(self, should_debuff)

    if self.ability.feli_fag_sgl_repulsive or self.ability.feli_fag_ttm_sgl_repulsive or SMODS.has_enhancement(self, "m_feli_fag_wild_t2") or SMODS.has_enhancement(self, "m_feli_fag_wild_t3") or SMODS.has_enhancement(self, "m_feli_fag_wild_t4") then
        self.debuff = false
        self.perma_debuff = false
    end

    if next(SMODS.find_card("j_feli_fag_productoffear")) and self.base.suit == "Hearts" then
        local lovers =
            {
                "c_lovers",
                "c_feli_fag_t2_lovers",
                "c_feli_fag_t2_lovers_mp",
                "c_feli_fag_t3_lovers",
                "c_feli_fag_t3_lovers_mp",
                "c_feli_fag_t4_lovers",
                "c_feli_fag_t4_lovers_mp",
                "c_bd_loverprints",
            }
        local debuff = false
        for _, v in ipairs(G.consumeables.cards) do
            for _, key in ipairs(lovers) do
                if v.config.center.key == key then
                    debuff = true
                    break
                end
            end
        end
        if debuff == true then
            self.debuff = false
            self.perma_debuff = false
        end
        
    end
end


local orig_card_drag = Card.drag
function Card:drag()
    if self.area and self.area == G.jokers then
        SMODS.calculate_context({dragging = true, drag_target = self})
    end
    local ret = orig_card_drag(self)
    return ret
end

local old_addtodeck = Card.add_to_deck
function Card:add_to_deck(from_debuff)
    if FelisAG.active_totem and self.ability.set == "Joker" then
        for _, _c in ipairs(G.feli_fag_totems.cards) do
		    if _c.ability.totem_active then
                G.E_MANAGER:add_event(Event({
                    func = function() 
                        FelisAG.applyTotemSigils(_c, _c.ability.totem_tribe)
                        return true
                    end
			    }))
            end
	    end
        
    end
    local ret = old_addtodeck(self, from_debuff)
    return ret
end


local cardSetCostHook = Card.set_cost
function Card:set_cost()
    local ret = cardSetCostHook(self)
    if self.config.center.key == "j_feli_fag_ins_ouro" and (self.ability.sell.curr >= self.ability.sell.limit) then
        self.cost = 0
        self.sell_cost = 0
    end
    if self.ability.feli_fag_copied then
        self.cost = 1
        self.sell_cost = 0
    end
    return ret
end


